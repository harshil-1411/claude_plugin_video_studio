...
export function createServer(options: ServerOptions = {}): McpServer {
  ...
  server.registerTool(
    "export",
    {
      ...
      description:
        "Rebuild <project_dir>/dist/ from the latest existing render (or the given quality) without rendering: reel.mp4, clean-master.mp4, captions.srt/.vtt, transcript.txt, thumbnail.png, social-copy.md (regenerated from the spec and brief; overwrite it after refining), render-manifest.json and provenance.json.",
      inputSchema: {
        project_dir: z.string().min(1).describe("Rendered project folder"),
