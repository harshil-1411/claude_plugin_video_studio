...
export function toAss(lines: readonly CaptionLine[], o: AssOptions): string {
  ...
  const { region } = layout;
  // With a plate, the box is its own layer: one "Plate" event per row with no style changes, so
  // libass draws exactly one box per row. Drawing it on the text itself (BorderStyle 3 on the
  // emphasised run too) gives each run its own box, and the translucent boxes overlap into dark
  // bars around highlighted words.
  const styleFields = (name: string, primary: string, secondary: string, borderStyle: number, outline: number, shadow: number) => [
    name,
    font,
    layout.fontSize,
    primary, // PrimaryColour: text (karaoke: the sweep fill)
    secondary, // SecondaryColour: karaoke text before the sweep
  ...
  const textPrimary = assColor(karaoke ? hiHex : baseHex);
  const textSecondary = assColor(baseHex);
  const style = layout.plate
    ? styleFields("Default", textPrimary, textSecondary, 1, 0, 0) // text only; the plate is its own layer
    : styleFields("Default", textPrimary, textSecondary, 1, layout.pad, st.shadow);
  const plateStyle = layout.plate ? styleFields("Plate", assColor("#FFFFFF00"), assColor("#FFFFFF00"), 3, layout.pad, 0) : undefined;
  const header = [
    "[Script Info]",
  ...
  const events = lines.flatMap((l) => {
    const t = `${assTimeCs(cs(l.start_ms))},${assTimeCs(cs(l.end_ms))}`;
    const text = `Dialogue: 1,${t},Default,,0,0,0,,${pos}${karaoke ? assKaraokeText(l, em, scriptFonts) : assStaticText(l, em, scriptFonts)}`;
    return plateStyle ? [`Dialogue: 0,${t},Plate,,0,0,0,,${pos}${assStaticText(l, undefined, scriptFonts)}`, text] : [text];
  });
