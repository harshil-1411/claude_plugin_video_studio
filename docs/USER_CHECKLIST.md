...
- ✅ **Marketplace:** done 2026-09-27 (README has the real `/plugin marketplace add harshil-1411/claude_plugin_video_studio` commands). replace `<owner>/<repo>` in the README install section and submit to the community marketplace.
...
- ✅ **Turn on the pre-push check (once):** done 2026-09-27. `pnpm hooks`. From then on every `git push` runs `pnpm check --push` first (about 3 minutes). `git push --no-verify` skips it in an emergency.
...
- ✅ **HyperFrames count-up and scene openings:** done 2026-09-27. render a project with a stat (e.g. 12 packages) and a few scenes using `--renderer hyperframes`. Run `review` in strip mode on the stat: digits count up, the unit stays put, and frame 0 of each scene isn't blank.
...
- ✅ **Approval dialog (MCP elicitation):** done 2026-09-27. in Claude Code, ask for a demo capture or a whisper model download. You should get the engine's own approval dialog (not just Claude asking in chat), and `project/consent.json` should record `via: elicitation`.
- ✅ **ElevenLabs prices:** done 2026-09-27. check `ELEVENLABS_PRICING` in `packages/voice/src/elevenlabs.ts` against elevenlabs.io/pricing/api (set in 2025, not verifiable from the sandbox). Spend limits in `policy.yaml` rely on it.
- ✅ **Video URL ingest:** done 2026-09-27.
...
- ✅ **Speakers:** done 2026-09-27. transcribe a two-person interview with `speakers: true`. It needs the ~488 MB English-only tinydiarize model, downloaded after your approval. Check that the S1/S2 labels follow the real turns.
